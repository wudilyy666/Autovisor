...
#### 2025/3/19 - 新增支持**从指定大课开始观看**的功能(用于因程序异常导致的被迫终止)
...
- `configs.ini`文件新增**从第几个大课开始观看**选项;
- 其他代码层面调整优化, 提高了程序稳定性.
...
- 在`configs.ini`文件中找到`startFromCourse = 1`这一行
- 将数字修改为你想从第几个大课开始观看，例如`startFromCourse = 3`表示从第3个大课开始
- 这个设置会跳过前面的大课，直接从指定序号的大课开始观看
- 如果设置的数字超出了实际大课数量，程序会自动从第1个大课开始
